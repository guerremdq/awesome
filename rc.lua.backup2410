...
-- Widgets using wicked, modified by Comtom

os.execute("/home/guerre/.awesomestart")
...
apptags =
{
    ...
    --["xchat"] = { screen = 1, tag = 4 },
  --  ["xterm"] = { screen = 1, tag = 1 }
    ["ocp"] = { screen = 1, tag = 1 },
    ["alsamixer"] = { screen = 1, tag = 1 },
    ["htop"] = { screen = 1, tag = 1 },
...
-- {{{ Tags
tags_names = {"xterm", "www", "im", "work", "media", "misc", "doc", "prog", "vbox"}
...
for s = 1, screen.count() do
    ...
    for tagnumber = 1, 9 do
        --tags[s][tagnumber] = tag(tagnumber) version mas vieja ! 
    ...
    tags[s][1].selected = true
     -- layout 5 para la tag 1
     awful.layout.set(layouts[5], tags[s][1])
    -- awful.layou.set(layouts[10], tags[s][2])
end
-- }}}
...
globalkeys = awful.util.table.join(
    ...
    awful.key({}, "XF86MonBrightnessUp",      function () awful.util.spawn(brilloup) end),
    awful.key({}, "XF86MonBrightnessDown",    function () awful.util.spawn(brillodown) end),
-- Prompt
    awful.key({ modkey },            "F1",     function () mypromptbox[mouse.screen]:run() end),
   
   -- Prompt
   --awful.key({ modkey }, "F2",                 function () awful.prompt({ prompt = "Run: ", cursor_fg = fg_focus, cursor_bg = bg_focus }, mypromptbox, awful.spawn, awful.completion.bash)
     --                           end),
